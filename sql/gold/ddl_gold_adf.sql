/* Script para Azure Data Factory (ADF)
Sem comandos 'GO' e preparado para execução via Script Activity
*/

-- 1. View: gold.view_salary_by_dept
IF OBJECT_ID('gold.view_salary_by_dept', 'V') IS NOT NULL DROP VIEW gold.view_salary_by_dept;
EXEC('
...
IF OBJECT_ID('gold.view_tenure', 'V') IS NOT NULL DROP VIEW gold.view_tenure;
EXEC('
CREATE VIEW gold.view_tenure AS
WITH tenure_calculation AS (
    SELECT employee_id, department, employment_type,
...
IF OBJECT_ID('gold.view_workforce_dashboard', 'V') IS NOT NULL DROP VIEW gold.view_workforce_dashboard;
EXEC('
CREATE VIEW gold.view_workforce_dashboard AS
WITH base_data AS (
    SELECT c.employee_id, c.department, c.position, c.employment_type, c.salary, c.admission_date,
...
SELECT 
    employee_id, country, gender, age_band, department, position, employment_type,
    tenure_days, retention_band,
    ...
    data_quality_status, is_birth_date_suspect, is_employment_type_adjusted,
